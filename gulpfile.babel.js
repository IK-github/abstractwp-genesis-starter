 ...
 */
gulp.task( 'styles', () => {
	return processStyle( gulp.src( config.styleSRC, { allowEmpty: true }), {
		styleDestination: config.styleDestination
	}).pipe(
		notify({ message: '\n\n✅  ===> STYLES — completed!\n', onLast: true })
	);
...
gulp.task( 'addonStyles', done => {
	...
	if ( 0 === config.addonStyles.length ) {
		return done();
	}

	// Process each addon style
	var tasks = config.addonStyles.map( function( addon ) {
		return processStyle( gulp.src( addon.styleSRC, { allowEmpty: true }), {
 ...
 */
gulp.task( 'stylesRTL', () => {
	return processStyleRTL( gulp.src( config.styleSRC, { allowEmpty: true }), {
		styleDestination: config.styleDestination
	}).pipe(
...
gulp.task( 'addonStylesRTL', done => {
	...
	// Process each addon style
	var tasks = config.addonStyles.map( function( addon ) {
		return processStyleRTL( gulp.src( addon.styleSRC, { allowEmpty: true }), {
			styleDestination: addon.styleDestination
		}).pipe(
			notify({
